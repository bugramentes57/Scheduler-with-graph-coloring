...
<body>
    ...
    if (isset($_POST["cmdaddnew"])) {
        $sql = "INSERT INTO sinif (sinif_id, sinif_ad)
                VALUES
                (:sinif_id, :sinif_ad)";
        ...
        $stmt->bindParam(':sinif_ad', $_POST["sinif_ad"]);
        

        if ($stmt->execute()) {
            echo "<script type= 'text/javascript'>alert('Kayıt başarıyla güncellendi');
                window.location.replace('edit3.php');</script>";
        } else {
    ...
    if ($editemployee == '') {
        ...
        $sql = "SELECT * FROM sinif WHERE sinif_id = :sinif_id";
        $stmt = $baglanti->prepare($sql);
        $stmt->bindParam(':sinif_id', $editemployee);
        $stmt->execute();

        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $sinif_id = $row["sinif_id"];
            $sinif_ad = $row["sinif_ad"];
    ...
    if (isset($_POST["cmdelete"])) {
        $sql = "DELETE FROM sinif WHERE sinif_id='$deleteemployee'";
    ...
    if (isset($_POST["cmdedit"])) {
        ...
        $sql = "UPDATE sinif SET sinif_id=:sinif_id, sinif_ad=:sinif_ad WHERE sinif_id=:sinif_id";

        $stmt = $baglanti->prepare($sql);
        $stmt->bindParam(':sinif_id', $sinif_id);
        $stmt->bindParam(':sinif_ad', $sinif_ad);
    ...
    <div class="container">
        ...
        <div class="table-wrapper">
            ...
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-6">
                        <h2>Sinif <b>Ekleme</b></h2>
                    </div>
                    <div class="col-sm-6">
                        <a href="#addEmployeeModal" class="btn btn-success" data-toggle="modal"><i
                                class="material-icons">🍳</i> <span>Sinif Ekle</span></a>
            ...
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    <?php
                    $urunsor = $baglanti->prepare("SELECT * FROM sinif");
                    $urunsor->execute();
                    while ($uruncek = $urunsor->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        <tr>
                            ...
                            <td>
                                <?PHP echo $uruncek['sinif_id'] ?>
                            </td>
                            <td>
                                <?PHP echo $uruncek['sinif_ad'] ?>
                            </td>
                            

                            <td>
                                <a href="?editemployee=<?php echo $uruncek['sinif_id']; ?>" class="edit"
                                    data-toggle="modal"><i class="material-icons" data-toggle="tooltip"
                                        title="Edit"></i></a>
                                <a href="?deleteemployee=<?php echo $uruncek['sinif_id']; ?>" class="delete"
                                    data-toggle="modal"><i class="material-icons" data-toggle="tooltip"
                                        title="Delete"></i></a>
                            </td>
                ...
                <div class="modal-content">
                    <form action="" method="post">
                        <div class="modal-header">
                            <h4 class="modal-title">Sinif Ekle</h4>
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                        </div>
                        <div class="modal-body">
                            <div class="form-group">
                                <label>sinif_id</label>
                                <input type="text" name="sinif_id" class="form-control" required>
                            </div>
                            <div class="form-group">
                                <label>sinif_ad</label>
                                <input type="text" name="sinif_ad" class="form-control" required>
                            </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <form name="frmedit" action="" method="post">
                        <div class="modal-header">
                            <h4 class="modal-title">Sinif Düzenleme</h4>
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                        </div>
                        <div class="modal-body">
                            <div class="form-group">
                                <label>sinif_id</label>
                                <input type="text" name="sinif_id" value="<?php echo $sinif_id; ?>"
                                    class="form-control" required>
                            </div>
                            <div class="form-group">
                                <label>sinif_ad</label>
                                <input type="text" name="sinif_ad" value="<?php echo $sinif_ad; ?>"
                                    class="form-control" required>
                            </div>
